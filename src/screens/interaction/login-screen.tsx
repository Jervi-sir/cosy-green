...
export function LoginScreen({ navigation }: NativeStackScreenProps<RootStackParamList, 'Login'>) {
  ...
  const [email, setEmail] = useState('user@costgreen.app');
  const [password, setPassword] = useState('12345678');

  return (
    <ScreenShell scroll>
      <HeroHeader title="تسجيل الدخول" subtitle="اختر ما إذا كان هذا الدخول سيفتح تطبيق المستخدم أو تطبيق الشاحنة." />
      <View style={styles.card}>
        <Text style={styles.label}>البريد الإلكتروني</Text>
        <TextInput value={email} onChangeText={setEmail} style={styles.input} placeholder="name@example.com" />
        <Text style={styles.label}>كلمة المرور</Text>
        <TextInput value={password} onChangeText={setPassword} style={styles.input} secureTextEntry placeholder="كلمة المرور" />
      </View>
      ...
      <PrimaryButton
        label={state.appRole === 'Truck' ? 'افتح تطبيق الشاحنة' : 'متابعة إعداد المستخدم'}
        onPress={() => navigation.navigate(state.appRole === 'Truck' ? 'TruckApp' : 'Subscription')}
      />
    </ScreenShell>
  );
}
